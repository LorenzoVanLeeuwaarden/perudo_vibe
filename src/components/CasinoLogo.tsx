...
export function CasinoLogo({ color }: CasinoLogoProps = {}) {
  ...
  // Get fixed color config if a color prop is provided
  const fixedColorConfig = color
    ? generateColorConfig(
        COLOR_CYCLE.find(c => c.name === color)?.hue ?? 45,
        COLOR_CYCLE.find(c => c.name === color)?.sat ?? 90,
        COLOR_CYCLE.find(c => c.name === color)?.light ?? 55
      )
    : null;
