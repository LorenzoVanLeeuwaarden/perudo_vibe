...
must_haves:
  truths:
    ...
    - "No animated text-shadow, filter, or backdrop-filter remains unguarded in codebase"
  artifacts:
...
Purpose: Extend Phase 13 optimization patterns to VictoryScreen, DefeatScreen, RevealPhase, CasinoLogo, and DyingDie. Add prefers-reduced-motion support for accessibility compliance (A11Y-01).
...
<task type="auto">
  <name>Task 3: Fix CasinoLogo and DyingDie with animation guards</name>
  <files>
    src/components/CasinoLogo.tsx
    src/components/DyingDie.tsx
  </files>
...
```bash
grep -n "animate.*textShadow" src/components/*.tsx
grep -n "animate.*filter" src/components/*.tsx
```
All matches should be inside conditional blocks or use `useSimplifiedAnimations`.
  </verify>
  <done>
CasinoLogo and DyingDie now have combined Firefox + reduced motion guards. No unguarded animated textShadow or filter properties remain in the codebase.
  </done>
</task>
