...
must_haves:
  truths:
    ...
    - "Tutorial uses isolated state - no impact on real game stats or achievements"
  artifacts:
...
- Tutorial does NOT import or use gauntletStore, achievementStore, or leaderboard
- Tutorial uses only tutorialStore for state management
- No localStorage keys are modified except tutorial-specific ones (tutorial_completed)
- Confirm component imports don't include any stores that track game stats
  </verify>
  <done>
...
- **GAME-05 Safe environment**: Tutorial uses isolated tutorialStore, doesn't affect real game state or achievements
</success_criteria>
