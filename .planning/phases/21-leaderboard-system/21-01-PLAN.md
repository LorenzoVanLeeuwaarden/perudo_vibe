...
must_haves:
  truths:
    ...
    - "D1 migration has been applied to the database"
  artifacts:
...
Note: Do NOT add cron triggers to wrangler.jsonc - PartyKit deployment doesn't support them directly.
  </action>
...
<checkpoint type="human-action" gate="blocking">
  <action>Apply D1 migration to database</action>
  <instructions>
After the D1 database is created and database_id is set in wrangler.jsonc, run the migration:
...
This creates the leaderboard table and indexes in the D1 database.
  </instructions>
  <resume-signal>Type "migrations applied" when both local and remote migrations are complete</resume-signal>
</checkpoint>
